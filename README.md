...
- Android Chrome の localhost 環境で、WebUI 全体を PWA としてインストール可能
...
- PWA の条件を満たす環境では、同じパネルに「Install App」ボタンが表示されます。
- ボタンクリックで操作パネルが出現します。

### 1.5. PWA としてインストール

- 初期対応ブラウザは Android Chrome です。
- installable になる条件は、localhost または HTTPS の secure context であることです。
- この拡張は WebUI 全体を PWA 化しますが、オフライン対応はキャッシュ済み UI と静的アセットの再表示に限定されます。
- 生成処理、履歴 API、外部画像取得は WebUI バックエンドが必要なため、オフラインでは利用できません。
- `Install App` ボタンが無効な場合は、Chrome 側のインストールプロンプトがまだ利用できないか、ブラウザメニューからのインストールに切り替わっています。
...
- PWA 用 service worker は拡張から配信されますが、キャッシュ対象は Mobile+ の静的資産と一部 UI シェルのみです
- 別端末から `http://192.168.x.x:7860` のような平文 HTTP で接続する場合、Android Chrome では PWA インストール対象になりません
